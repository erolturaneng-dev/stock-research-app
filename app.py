...
def calculate_overall_score(info, scoring, contract_summary):
    ...
    components_df = pd.DataFrame(
        [
            ...
            {"Bileşen": "Güvenlik Skoru (100 - Risk)", "Skor": round(safety_score, 1), "Ağırlık": "%25"},
...
def render_quick_company_analysis(selected_ticker):
    ...
    st.dataframe(selected_components_df, use_container_width=True, hide_index=True)
    ...
    if selected_hist is not None and not selected_hist.empty:
        ...
        if len(selected_hist) >= 50:
            ...
            fig2.add_trace(go.Scatter(x=selected_hist.index, y=ma50, mode="lines", name="MA50"))

        if len(selected_hist) >= 200:
            ma200 = selected_hist["Close"].rolling(200).mean()
            fig2.add_trace(go.Scatter(x=selected_hist.index, y=ma200, mode="lines", name="MA200"))
