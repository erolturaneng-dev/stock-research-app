...
def _component_comment(name, score):
    ...
    if "Güvenlik" in name:
        return "Risk daha kontrollü görünüyor." if score >= 70 else "Risk seviyesi orta. Dikkatli takip edilmeli." if score >= 40 else "Risk yüksek görünüyor."
    if "Momentum" in name:
        return "Teknik görünüm ve fiyat momentumu güçlü." if score >= 70 else "Momentum karışık / nötr." if score >= 40 else "Momentum zayıf."
    if "Finansal" in name:
        return "Finansal kalite güçlü." if score >= 70 else "Finansal kalite karışık." if score >= 40 else "Finansal kalite zayıf."
    if "Kontrat" in name:
        return "Katalizör / kontrat tarafı güçlü." if score >= 70 else "Katalizör tarafı orta." if score >= 40 else "Katalizör desteği zayıf."
    return ""
...
def render_component_score_cards(components_df):
    ...
    for i, row in components_df.iterrows():
        ...
        with cols[i % 2]:
            st.markdown(
                f"""
                <div style="background-color:{bg}; border:2px solid {color}; border-radius:14px; padding:16px; margin-bottom:14px; box-shadow:0 1px 4px rgba(0,0,0,0.06);">
                    <div style="display:flex; justify-content:space-between; align-items:center; gap:10px;">
                        <div style="font-size:18px; font-weight:700; color:#111;">{name}</div>
                        <div style="font-size:13px; font-weight:700; color:{text_color}; background:rgba(255,255,255,0.65); padding:4px 10px; border-radius:999px; white-space:nowrap;">{level} · {weight}</div>
                    </div>
                    <div style="font-size:34px; font-weight:800; color:{color}; margin-top:8px;">{score_int}/100</div>
                    <div style="height:10px; background-color:white; border-radius:99px; overflow:hidden; margin:8px 0 10px 0;">
                        <div style="width:{score_int}%; height:10px; background-color:{color}; border-radius:99px;"></div>
                    </div>
                    <div style="font-size:14px; color:{text_color}; line-height:1.45;">{comment}</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
...
def get_opportunity_card_style(opportunity_score, risk_score):
    ...
    if opportunity_score >= 70 and risk_score <= 60:
        return {"border": "#16a34a", "bg": "#ecfdf3", "text": "#166534", "label": "Güçlü Fırsat", "emoji": "🟢"}
    if opportunity_score >= 50 and risk_score <= 75:
        return {"border": "#eab308", "bg": "#fef9c3", "text": "#854d0e", "label": "İzlenebilir Aday", "emoji": "🟡"}
    return {"border": "#ef4444", "bg": "#fee2e2", "text": "#991b1b", "label": "Riskli / Zayıf", "emoji": "🔴"}


def render_opportunity_candidate_cards(df):
    st.markdown("---")
    st.markdown("## Fırsat Listesi")
    st.caption("Adaylar fırsat skoruna göre sıralanır. Yeşil güçlü, sarı izlenebilir, kırmızı daha riskli/zayıf görünümü gösterir.")

    display_df = df.copy().sort_values(["Opportunity Score", "Risk Score", "Piyasa Değeri Raw"], ascending=[False, True, True], na_position="last").reset_index(drop=True)
    ...
    for i, row in display_df.iterrows():
        ...
        with st.container(border=True):
            st.markdown(
                f"""
                <div style="background:{style['bg']}; border-left:8px solid {style['border']}; border-radius:12px; padding:16px; margin-bottom:8px;">
                    <div style="display:flex; justify-content:space-between; align-items:center; gap:10px; flex-wrap:wrap;">
                        <div style="font-size:22px; font-weight:800;">{style['emoji']} {ticker} — {company}</div>
                        <div style="background:white; border:1px solid {style['border']}; color:{style['text']}; border-radius:999px; padding:6px 12px; font-weight:700;">{style['label']}</div>
                    </div>
                    <div style="font-size:14px; color:#555; margin-top:6px;">{tags}</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
            ...
            if notes:
                st.caption(notes)
            if st.button(f"{ticker} için Tek Şirket Analizine Git", key=f"go_single_{ticker}_{i}", type="primary"):
                st.session_state.jump_to_single_ticker = ticker
                st.rerun()


def render_quick_company_analysis(selected_ticker):
    selected_ticker = clean_ticker(selected_ticker)
    with st.spinner(f"{selected_ticker} detaylı analiz ediliyor..."):
        ...
        selected_overall_score, selected_overall_style, selected_components_df, selected_detail_notes = calculate_overall_score(selected_info, selected_scoring, selected_contract_summary)
    ...
    with gg1:
        st.plotly_chart(create_overall_score_gauge(selected_overall_score), use_container_width=True)
    with gg2:
    ...
    if selected_hist is not None and not selected_hist.empty:
        fig2 = go.Figure()
        fig2.add_trace(go.Scatter(x=selected_hist.index, y=selected_hist["Close"], mode="lines", name="Kapanış"))
        if len(selected_hist) >= 50:
            fig2.add_trace(go.Scatter(x=selected_hist.index, y=selected_hist["Close"].rolling(50).mean(), mode="lines", name="MA50"))
        if len(selected_hist) >= 200:
            fig2.add_trace(go.Scatter(x=selected_hist.index, y=selected_hist["Close"].rolling(200).mean(), mode="lines", name="MA200"))
    ...
    if selected_overall_score >= 85:
        st.success("Çok güçlü aday. Şirket hem fırsat hem genel kalite açısından güçlü görünüyor. Yine de bilanço, SEC dosyaları ve haber akışı manuel doğrulanmalı.")
    elif selected_overall_score >= 70:
        st.success("Güçlü aday. Derin araştırmaya değer görünüyor ve watchlist içinde üst sıralarda tutulabilir.")
    elif selected_overall_score >= 40:
        st.warning("Karışık görünüm. Şirket izlenebilir ama tablo henüz tam güçlü değil. Özellikle finansal kalite ve risk tarafı dikkatle incelenmeli.")
    else:
        st.error("Zayıf / riskli görünüm. Şu aşamada öncelikli aday gibi görünmüyor.")
...
with st.sidebar:
    ...
    if st.session_state.watchlist:
        ...
        st.download_button("Watchlist CSV indir", data=wl_df.to_csv(index=False).encode("utf-8"), file_name="watchlist.csv", mime="text/csv")
...
st.info("Bu platform yatırım tavsiyesi değildir. Amaç; erken aşama teknoloji hisselerini filtrelemek, riskleri görmek ve derin araştırma adayları oluşturmaktır.")

if mode == "Fırsat Tarayıcısı":
    st.header("Fırsat Tarayıcısı")
    st.write("Bu bölüm, BBAI / erken dönem PLTR benzeri potansiyel teknoloji hisselerini bulmak için fiyat, piyasa değeri, büyüme, momentum, sektör etiketi ve finansal risk filtreleri kullanır.")

    with st.sidebar:
        st.markdown("### Tarama Ayarları")
        scan_scope = st.selectbox("Tarama kapsamı", ["Stratejik teknoloji aday listesi", "Geniş ABD teknoloji taraması"], index=0)
        scan_preset = st.selectbox("Filtre tipi", ["Dengeli fırsat avı", "Agresif küçük hisse avı", "Daha güvenli small/mid cap", "AI / savunma / uzay odaklı", "Özel ayarlar"], index=0)
        ...
        max_scan = st.slider("Geniş taramada en fazla kaç şirket incelensin?", min_value=100, max_value=2000, value=500, step=100)
        extra_tickers = st.text_input("Her zaman eklenecek hisse kodları", value="BBAI,SOUN,AISP,PLTR,RKLB,RDW,LUNR,QUBT,RGTI,IONQ")
        run_opportunity_scan = st.button("Fırsatları Tara", type="primary")

    if not run_opportunity_scan:
        st.markdown("## Platform Özeti")
        st.write("Bu platform; AI, uzay, savunma teknolojileri, kuantum, siber güvenlik, enerji altyapısı, robotik ve yarı iletken sektörlerindeki halka açık ABD şirketlerini araştırmak için tasarlanmıştır.")
        c1, c2, c3 = st.columns(3)
        with c1:
            st.markdown("#### Fırsat Tarayıcısı")
            st.write("Erken aşama teknoloji fırsatlarını bulmak için fiyat, piyasa değeri, büyüme, momentum, stratejik etiket ve finansal risk filtreleri kullanır.")
        with c2:
            st.markdown("#### Sektör Tarayıcısı")
            st.write("AI, Space, Defense, Quantum, Cybersecurity, Energy, Robotics ve Semiconductor alanlarını Small Cap, Mid Cap ve Large Cap olarak ayırır.")
        with c3:
            st.markdown("#### Tek Şirket Analizi")
            st.write("Bir hisse kodu için finansal durum, fiyat grafiği, momentum, kontratlar, SEC dosyaları ve 6-12 aylık senaryo analizi üretir.")
        ...
        st.info("Başlamak için sol menüden **Tarama kapsamı** seç. Önerilen başlangıç: **Stratejik teknoloji aday listesi** + **Dengeli fırsat avı**.")
    ...
    if run_opportunity_scan:
        with st.spinner("Tarama listesi hazırlanıyor..."):
            if scan_scope == "Stratejik teknoloji aday listesi":
                ...
                scan_df = build_broad_tech_universe(symbols_df=symbols_df, max_candidates=max_scan, extra_tickers=extra_tickers)
        ...
        if scan_df.empty:
            ...
            for count, (_, row) in enumerate(scan_df.iterrows(), start=1):
                ...
                if sector is None and industry is None:
                    progress.progress(count / total_scan); continue
            ...
            if not rows:
                st.warning("Bu taramada filtrelere uyan aday bulunamadı. Filtreleri genişletmeyi veya diğer tarama kapsamını seçmeyi deneyebilirsin.")
            else:
                ...
                df = df.sort_values(["Opportunity Score", "Risk Score", "Piyasa Değeri Raw"], ascending=[False, True, True], na_position="last")

                display_cols = ["Hisse Kodu", "Şirket Adı", "CIK", "Borsa", "Kaynak", "Sektör", "Endüstri", "Stratejik Etiket", "Piyasa Değeri", "Piyasa Değeri Grubu", "Hisse Fiyatı", "Gelir Büyümesi", "Kâr Marjı", "Free Cash Flow", "Toplam Nakit", "Toplam Borç", "Beta", "Opportunity Score", "Risk Score", "Momentum Score", "Araştırma Etiketi", "Skor Notları"]
                ...
                tab1, tab2, tab3, tab4 = st.tabs(["High Potential", "Watchlist", "Small / Micro", "Tüm Sonuçlar"])
                with tab1:
                ...
                st.download_button(label="Adayları CSV indir", data=csv_data, file_name=f"opportunity_candidates_{datetime.today().strftime('%Y%m%d')}.csv", mime="text/csv")
                st.success(f"Bulunan aday sayısı: {len(df)}")
                top = df.iloc[0]
                st.info(f"En yüksek skorlu aday: **{top['Hisse Kodu']} - {top['Şirket Adı']}** Opportunity Score: **{top['Opportunity Score']}/100**, Risk Score: **{top['Risk Score']}/100**")
                st.warning("Düşük fiyat tek başına fırsat değildir. Nano-cap, zarar eden ve nakit yakan şirketlerde dilution, delisting, likidite ve manipülasyon riski yüksek olabilir.")
    ...
    if run_sector_scan:
        ...
        df = pd.DataFrame(rows)
        display_cols = ["Hisse Kodu", "Şirket Adı", "Sektör", "Endüstri", "Stratejik Etiket", "Piyasa Değeri", "Piyasa Değeri Grubu", "Hisse Fiyatı", "Gelir Büyümesi", "Kâr Marjı", "Opportunity Score", "Risk Score", "Momentum Score", "Araştırma Etiketi", "Skor Notları"]
        small_df = df[df["Piyasa Değeri Grubu"] == "Small Cap / Erken Aday"].sort_values("Opportunity Score", ascending=False)
        mid_df = df[df["Piyasa Değeri Grubu"] == "Mid Cap / Büyüme Adayı"].sort_values("Opportunity Score", ascending=False)
        large_df = df[df["Piyasa Değeri Grubu"] == "Large Cap / Sektör Lideri"].sort_values("Piyasa Değeri Raw", ascending=False)

        tab1, tab2, tab3, tab4 = st.tabs(["Small Cap / Early", "Mid Cap / Growth", "Large Cap / Leaders", "Tümü"])
        with tab1:
        ...
        with tab4:
            st.subheader("Tüm Sonuçlar")
            st.dataframe(df.sort_values("Opportunity Score", ascending=False)[display_cols], use_container_width=True)

        st.download_button(label="Sektör sonuçlarını CSV indir", data=df[display_cols].to_csv(index=False).encode("utf-8"), file_name=f"{selected_sector}_sector_scan_{datetime.today().strftime('%Y%m%d')}.csv", mime="text/csv")

elif mode == "Tek Şirket Analizi":
    st.header("Tek Şirket Analizi")

    with st.sidebar:
        ticker = st.text_input("Hisse kodu girin", value=st.session_state.selected_single_ticker)
        company_name_manual = st.text_input("Şirket adı girin", value="")
        selected_sector = st.selectbox("Sektör seçin", list(SECTOR_UNIVERSE.keys()), index=0)
        run_single = st.button("Şirketi Analiz Et", type="primary")

    should_auto_run = bool(st.session_state.auto_run_single)
    st.session_state.auto_run_single = False
